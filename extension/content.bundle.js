(()=>{var N="https://api.damkoi.com";async function S(i,t={}){return new Promise((e,a)=>{chrome.runtime.sendMessage({type:i,...t},r=>{chrome.runtime.lastError?a(new Error(chrome.runtime.lastError.message)):r&&r.success?e(r.data):a(new Error(r?.error||"Unknown error"))})})}var pt={EMAIL:"email",SMS:"sms",PUSH:"push"},ut=pt.EMAIL,mt=60*60*1e3;function z(i,t){return`damkoi:${i}:${t}`}function P(i){try{let t=localStorage.getItem(i);if(!t)return null;let{data:e,timestamp:a}=JSON.parse(t);return Date.now()-a>mt?(localStorage.removeItem(i),null):e}catch(t){return console.warn("[DamKoi Cache] Failed to read cache:",t),null}}function R(i,t){try{localStorage.setItem(i,JSON.stringify({data:t,timestamp:Date.now()}))}catch(e){console.warn("[DamKoi Cache] Failed to save cache:",e)}}function G(i){return i>=8?"#10b981":i>=6?"#f59e0b":i>=4?"#ef4444":"#dc2626"}function W(i){return i>=8?"score-green":i>=6?"score-amber":"score-red"}function f(i){return i?`\u09F3${(i/100).toLocaleString("en-BD")}`:"\u2014"}function V(i,t,e,a=[ut]){return{product_id:i,target_price:parseInt(t)*100,email:e.trim(),notify_via:a}}function C(i,t,e){if(!i)return;let a=t==="success",r=t==="info";i.textContent=e,i.classList.remove("dk-status-success","dk-status-error","dk-status-info"),a?i.classList.add("dk-status-success"):r?i.classList.add("dk-status-info"):i.classList.add("dk-status-error")}function J(i){return i&&/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(i.trim())}function X(i){let t=parseInt(i);return!isNaN(t)&&t>0}var ht={renderDealGauge(i,t){let e=document.getElementById(t);if(!e)return;let r=Math.min(Math.max(i,0),10)/10,d=G(i),m=Math.PI*40,n=m*(1-r);e.innerHTML=`
    ...
    `},renderPriceChart(i,t){let e=document.getElementById(t);if(!e||!i||i.length<2){e.innerHTML='<div class="damkoi-no-data">Not enough data for chart</div>';return}let a=[...i].sort((k,b)=>new Date(k.scraped_at)-new Date(b.scraped_at)),r=a.map(k=>k.price),d=Math.min(...r)*.95,m=Math.max(...r)*1.05-d,n=280,c=120,p=10,l=n-p*2,u=c-p*2,h=a.map((k,b)=>{let T=p+b*(l/(a.length-1)),B=p+(u-(k.price-d)/m*u);return{x:T,y:B}}),v=h.map((k,b)=>`${b===0?"M":"L"} ${k.x} ${k.y}`).join(" "),A=`${v} L ${h[h.length-1].x} ${c-p} L ${h[0].x} ${c-p} Z`;e.innerHTML=`
      <div class="damkoi-chart-container">
        <svg width="${n}" height="${c}" viewBox="0 0 ${n} ${c}">
          ...
          <path d="${v}" fill="none" stroke="#a78bfa" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" />

          <!-- Tooltip dots for each point (hidden by default) -->
          ${h.map((k,b)=>`
            <circle cx="${k.x}" cy="${k.y}" r="3" fill="#fff" class="damkoi-chart-dot" opacity="${b===h.length-1?1:0}">
              <title>${new Date(a[b].scraped_at).toLocaleDateString()}: ${f(a[b].price)}</title>
            </circle>
          `).join("")}
        </svg>
      </div>
    `}},j=ht;var x={summary:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>',history:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>',alternatives:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 3h5v5"/><path d="M8 3H3v5"/><path d="M12 21v-8"/><path d="m7 16 5 5 5-5"/></svg>',alerts:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9"/><path d="M10.3 21a1.94 1.94 0 0 0 3.4 0"/></svg>',close:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>'};var Y="damkoi-inline-root";var gt=['[class*="pdp-product-price"]','[class*="product-price"]','[class*="pdp-info-block"]',".pdp-product-main--price",'[class*="pdp-block"]',"#module_add_to_cart",'[class*="add-to-cart"]','form[action*="cart"]','[class*="pdp-product-detail"]'],s={bg:"#FAFAF9",raised:"rgba(255, 255, 255, 0.45)",inset:"rgba(0, 0, 0, 0.04)",border:"rgba(255, 255, 255, 0.5)",accent:"#A16207",success:"#059669",danger:"#DC2626",warn:"#D97706",text:"#0C0A09",muted:"#44403C",dim:"#A8A29E"},F={BEST_PRICE:{icon:"\u{1F3C6}",label:"Best Price",color:s.success,bg:"rgba(16,185,129,0.12)",rec:"buy"},GOOD_DEAL:{icon:"\u2705",label:"Good Deal",color:"#34d399",bg:"rgba(52,211,153,0.1)",rec:"buy"},FAIR_PRICE:{icon:"\u{1F7E1}",label:"Fair Price",color:s.warn,bg:"rgba(245,158,11,0.1)",rec:"wait"},FAKE_DISCOUNT:{icon:"\u274C",label:"Fake Discount",color:s.danger,bg:"rgba(239,68,68,0.1)",rec:"wait"},INSUFFICIENT_DATA:{icon:"\u{1F4CA}",label:"Not Enough Data",color:s.muted,bg:"rgba(123,123,158,0.1)",rec:null}};function vt(i){return i>=9?s.success:i>=7?"#34d399":i>=5?s.warn:s.danger}function M(i,t=480,e=130){if(!i||i.length<2)return{svg:`<div style="text-align:center;color:${s.dim};font-size:12px;padding:28px 0;">
              Not enough price data yet \u2014 check back in a few hours as we build your history.
            </div>`,initInteractivity:()=>{}};let a=[...i].sort((g,y)=>new Date(g.scraped_at)-new Date(y.scraped_at)),r=a.map(g=>g.price),d=Math.min(...r),o=Math.max(...r),m=o-d||1,n={top:12,right:56,bottom:26,left:10},c=t-n.left-n.right,p=e-n.top-n.bottom,l=g=>n.left+g/(a.length-1)*c,u=g=>n.top+p-(g-d)/m*p,h=a.map((g,y)=>`${y===0?"M":"L"}${l(y).toFixed(1)},${u(g.price).toFixed(1)}`).join(" "),v=`${h} L${l(a.length-1).toFixed(1)},${(n.top+p).toFixed(1)} L${n.left},${(n.top+p).toFixed(1)} Z`,k=[0,Math.floor(a.length/2),a.length-1].map(g=>{let $=new Date(a[g].scraped_at).toLocaleDateString("en-BD",{month:"short",day:"numeric"});return`<text x="${l(g).toFixed(1)}" y="${e-5}" text-anchor="middle"
              fill="${s.dim}" font-size="9" font-family="Inter,sans-serif">${$}</text>`}).join(""),b=u(d),T=u(o),B=r.lastIndexOf(d),O=`dkg_${Math.random().toString(36).slice(2,7)}`,et=a.map((g,y)=>{let $=g.price===d&&y===B,E=y===a.length-1;if(!$&&!E)return"";let w=$?s.success:s.accent,D=E?5:4;return`<circle cx="${l(y).toFixed(1)}" cy="${u(g.price).toFixed(1)}" r="${D}"
    ...
    <svg id="${_}" width="${t}" height="${e}" viewBox="0 0 ${t} ${e}"
      ...
      <!-- Grid lines -->
      <line x1="${n.left}" y1="${T.toFixed(1)}" x2="${t-n.right}" y2="${T.toFixed(1)}"
            stroke="rgba(0,0,0,0.06)" stroke-dasharray="3 3" stroke-width="1"/>
      <line x1="${n.left}" y1="${b.toFixed(1)}" x2="${t-n.right}" y2="${b.toFixed(1)}"
            stroke="rgba(16,185,129,0.12)" stroke-dasharray="3 3" stroke-width="1"/>

      <!-- Area + line -->
      <path d="${v}" fill="url(#${O})"/>
      <path d="${h}" fill="none" stroke="${s.accent}" stroke-width="2"
            stroke-linecap="round" stroke-linejoin="round"/>

      <!-- Price labels right side -->
      <text x="${t-n.right+4}" y="${(T+4).toFixed(1)}"
            fill="${s.muted}" font-size="9" font-family="Inter,sans-serif">${f(o)}</text>
      <text x="${t-n.right+4}" y="${(b+4).toFixed(1)}"
            fill="${s.success}" font-size="9" font-family="Inter,sans-serif">${f(d)}</text>
    ...
    "></div>`;function at(g){let y=g.getElementById(_),$=g.getElementById(`${_}_hline`),E=g.getElementById(`${_}_hdot`),w=g.getElementById(`${_}_tip`);if(!y||!$||!E||!w)return;let D=y.parentElement;D&&(D.style.position="relative"),y.addEventListener("mousemove",st=>{let I=y.getBoundingClientRect(),rt=(st.clientX-I.left)/I.width*t,ot=Math.max(n.left,Math.min(t-n.right,rt)),nt=Math.round((ot-n.left)/c*(a.length-1)),q=Math.max(0,Math.min(a.length-1,nt)),H=a[q],L=l(q),U=u(H.price);$.setAttribute("x1",L.toFixed(1)),$.setAttribute("x2",L.toFixed(1)),$.setAttribute("opacity","0.7"),E.setAttribute("cx",L.toFixed(1)),E.setAttribute("cy",U.toFixed(1)),E.setAttribute("opacity","1");let dt=new Date(H.scraped_at).toLocaleDateString("en-BD",{month:"short",day:"numeric",year:"numeric"});w.innerHTML=`<span style="color:${s.dim}">${dt}</span>&nbsp;&nbsp;<strong style="color:${s.accent}">${f(H.price)}</strong>`,w.style.display="block";let ct=L/t*I.width+(L>t*.65?-(180+8):12),lt=U/e*I.height-16;w.style.left=`${ct}px`,w.style.top=`${lt}px`}),y.addEventListener("mouseleave",()=>{$.setAttribute("opacity","0"),E.setAttribute("opacity","0"),w.style.display="none"})}return{svg:it,initInteractivity:at}}var kt=`
...
`;function ft(i){return!i||i.length===0?"":`
    ...
    </div>`}function xt(i){let t=vt(i),a=Math.PI*24,r=a*(1-i/10);return`
  ...
  <div id="dkw">
    ...
    <!-- Price stats (4 tiles) -->
    <div class="dk-stats">
      ${[["Current",f(d.current_price),s.accent],["30-Day Avg",o.avg_30d?f(o.avg_30d):"\u2014",s.text],["All-Time Low",o.all_time_low?f(o.all_time_low):"\u2014",s.success],["Highest Ever",o.all_time_high?f(o.all_time_high):`${p.length>0?f(Math.max(...p.map(l=>l.price))):"\u2014"}`,s.muted]].map(([l,u,h])=>`
        <div class="dk-stat">
          <div class="dk-stat-label">${l}</div>
          <div class="dk-stat-value" style="color:${h};">${u}</div>
    ...
    `,document.body.appendChild(t),this._updateCouponWidgetLabel(),document.getElementById("dk-apply-btn").onclick=async()=>{let e=document.getElementById("dk-apply-btn");e.innerText="Testing Coupons...",e.style.background="#4f46e5",e.style.opacity="0.7",e.disabled=!0;try{let a=window.location.hostname.includes("daraz"),r=await S("FETCH_COUPONS",{platform:a?"daraz":"pickaboo",paymentMethod:this._paymentMethod||void 0});if(!r||r.length===0){e.innerText="No valid coupons found",e.disabled=!1;return}let d=a?'input[placeholder*="oupon" i], input[name*="coupon" i], .next-input.next-medium input':'input[placeholder*="oupon" i], input[name*="coupon" i], .coupon-input input, #coupon-code',o=a?'button[data-spm*="coupon"], .next-btn.next-btn-primary.next-btn-medium, button[class*="couponApply"]':'button[class*="coupon"], .apply-coupon-btn, button[id*="couponApply"]',m=a?'.checkout-order-total-discount, [class*="discount"][class*="total"], [class*="coupon"][class*="discount"]':'.order-summary .discount-amount, [class*="discount-amount"]',n=0,c=null;for(let p=0;p<r.length;p++){let l=r[p];e.innerText=`Testing ${l.code} (${p+1}/${r.length})`;let u=document.querySelector(d),h=document.querySelector(o);if(u&&h){u.value=l.code,u.dispatchEvent(new Event("input",{bubbles:!0})),u.dispatchEvent(new Event("change",{bubbles:!0})),h.click(),await new Promise(A=>setTimeout(A,2e3));let v=document.querySelector(m);if(v){let A=v.innerText.match(/[\d,]+/);if(A){let k=parseInt(A[0].replace(/,/g,""));k>n&&(n=k,c=l.code)}}}}if(c){e.innerText=`Applying best: ${c}`;let p=document.querySelector(d),l=document.querySelector(o);p&&l&&(p.value=c,p.dispatchEvent(new Event("input",{bubbles:!0})),l.click(),await new Promise(u=>setTimeout(u,1e3))),e.innerText=`Saved \u09F3${n.toLocaleString("en-BD")}!`,e.style.background="#10b981",e.disabled=!1}else e.innerText="No coupons worked",e.style.background="#3f3f46",e.disabled=!1}catch(a){console.error("[DamKoi]",a),e.innerText="Error trying coupons",e.disabled=!1}}}renderSidebar(){this.sidebar&&this.sidebar.remove(),this.sidebar=document.createElement("div"),this.sidebar.id="damkoi-sidebar",this.sidebar.innerHTML=`
      ...
      <div class="damkoi-card" style="margin-bottom:10px;display:flex;gap:12px;cursor:pointer;" data-alt-url="${o.url}" data-alt-idx="${m}">
        ...
        <div style="flex:1;min-width:0;">
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;">
            <span style="font-size:13px;font-weight:800;color:var(--dk-text);">${f(o.current_price)}</span>
            ${n>0?`<span style="font-size:10px;background:rgba(16,185,129,0.1);color:var(--dk-success);padding:2px 7px;border-radius:10px;font-weight:600;">Save ${f(n)}</span>`:""}
          </div>
        </div>
      </div>
    ...
    `;let d=t.querySelector("#save-alert"),o=t.querySelector("#alert-email"),m=t.querySelector("#alert-price");d.replaceWith(d.cloneNode(!0)),t.querySelector("#save-alert").addEventListener("click",async()=>{let c=o.value.trim(),p=m.value.trim(),l=t.querySelector("#alert-status");if(!J(c)){C(l,"error","Enter a valid email address");return}if(!X(p)){C(l,"error","Enter a valid price");return}this.alertFormState={email:c,price:p},C(l,"info","Saving...");try{let u=V(this.data.product.id,p,c);await S("CREATE_ALERT",{payload:u}),C(l,"success","Tracking active!")}catch{C(l,"error","Error saving alert")}})}},tt=new K;document.readyState==="complete"?tt.init():window.addEventListener("load",()=>tt.init());})();
