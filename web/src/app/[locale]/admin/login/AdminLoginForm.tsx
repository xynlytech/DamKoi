...
export default function AdminLoginForm({ notAdmin = false }: { notAdmin?: boolean }) {
  const router = useRouter();
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [showPw, setShowPw] = useState(false);
  const [status, setStatus] = useState<"idle" | "loading" | "err">(notAdmin ? "err" : "idle");
  const [msg, setMsg] = useState(notAdmin ? "Your account does not have admin access." : "");
