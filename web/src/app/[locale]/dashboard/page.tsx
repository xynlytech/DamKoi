...
function ProductCard({ p }: { p: Product }) {
  const color = PLATFORM_COLOR[p.platform] ?? "rgba(255,255,255,0.4)";
  return (
    <Link href={`/product/${p.id}`} className="dk-card p-4 flex gap-3 group block">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-sm font-medium line-clamp-2 leading-snug mt-0.5" style={{ color: "rgba(255,255,255,0.8)" }}>{p.title}</p>
      ...
      <ArrowUpRight size={14} className="flex-shrink-0 mt-1 transition-colors" style={{ color: "rgba(255,255,255,0.1)" }}
...
function AlertRow({ a }: { a: Alert }) {
  ...
  return (
    <Link href={`/product/${a.product_id}`} className="flex items-center gap-3 py-3 transition-colors group" style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
      {!a.current_price ? <Minus size={14} style={{ color: "rgba(255,255,255,0.2)" }} />
        : hit ? <TrendingDown size={14} style={{ color: "var(--green)" }} />
        : <TrendingUp size={14} style={{ color: "var(--amber)" }} />}
      <div className="flex-1 min-w-0">
        <p className="text-sm line-clamp-1 leading-snug" style={{ color: "rgba(255,255,255,0.7)" }}>{a.product_title ?? "Product"}</p>
        <p className="text-[10px] mt-0.5" style={{ color: "rgba(255,255,255,0.3)" }}>
          Target: <span style={{ color: "rgba(255,255,255,0.6)" }}>{fmt(a.target_price)}</span>
          {a.current_price && <> · Now: <span style={{ color: hit ? "var(--green)" : "rgba(255,255,255,0.6)", fontWeight: hit ? 600 : 400 }}>{fmt(a.current_price)}</span></>}
        </p>
      </div>
      ...
      {!a.is_active && <span className="text-[9px] font-medium px-2 py-0.5 rounded-full flex-shrink-0" style={{ color: "rgba(255,255,255,0.2)", background: "rgba(255,255,255,0.05)" }}>Paused</span>}
    </Link>
  );
}
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-white flex items-center gap-3">
            <LayoutDashboard size={26} style={{ color: "#a78bfa" }} />
            Dashboard
          </h1>
        ...
        <button
          ...
          style={{ border: "1px solid var(--border-sm)", background: "var(--bg1)", color: "rgba(255,255,255,0.4)" }}
        >
      ...
      >
        {[
          ...
          <motion.div key={s.label} variants={item} className="dk-stat-card text-center">
            ...
            <div className="text-[9px] mt-0.5" style={{ color: "rgba(255,255,255,0.2)" }}>{s.sub}</div>
          </motion.div>
        ))}
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xs font-semibold uppercase tracking-widest" style={{ color: "rgba(255,255,255,0.4)" }}>All tracked products</h2>
            <Link href="/" className="text-xs transition-colors dk-focus" style={{ color: "#a78bfa" }}>+ Track another</Link>
          </div>

          {loadingProd ? (
            <div className="flex justify-center py-16"><Loader2 size={22} className="animate-spin" style={{ color: "#a78bfa" }} /></div>
          ) : products.length === 0 ? (
            <div className="text-center py-16 rounded-2xl" style={{ background: "var(--bg1)", border: "1px solid var(--border-sm)" }}>
              <Activity size={44} strokeWidth={1.5} className="mx-auto mb-4" style={{ color: "rgba(255,255,255,0.2)" }} />
              <h3 className="font-semibold text-white mb-2">Nothing tracked yet</h3>
              <p className="text-sm mb-6" style={{ color: "rgba(255,255,255,0.4)" }}>Paste a product URL on the homepage to start.</p>
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xs font-semibold uppercase tracking-widest flex items-center gap-2" style={{ color: "rgba(255,255,255,0.4)" }}>
              <Bell size={13} /> My Alerts
            </h2>
            <Link href="/alerts" className="text-xs transition-colors dk-focus" style={{ color: "#a78bfa" }}>Manage</Link>
          </div>

          <div className="rounded-2xl p-4" style={{ background: "var(--bg1)", border: "1px solid var(--border-sm)" }}>
            {loadingAlert ? (
              <div className="flex justify-center py-8"><Loader2 size={18} className="animate-spin" style={{ color: "#a78bfa" }} /></div>
            ) : alerts.length === 0 ? (
              <div className="text-center py-6">
                <BellOff size={36} strokeWidth={1.5} className="mx-auto mb-3" style={{ color: "rgba(255,255,255,0.2)" }} />
                <p className="text-sm mb-4" style={{ color: "rgba(255,255,255,0.4)" }}>No alerts set yet.</p>
                <Link href="/" className="text-xs dk-focus" style={{ color: "#a78bfa" }}>Find a product →</Link>
              </div>
            ) : (
              <div>
                {alerts.slice(0, 5).map((a) => <AlertRow key={a.id} a={a} />)}
                {alerts.length > 5 && (
                  <Link href="/alerts" className="block text-center text-xs mt-3 transition-colors dk-focus" style={{ color: "#a78bfa" }}>
